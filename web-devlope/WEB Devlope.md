...
<meta
  name="viewport"
  content="width=device-width,height=device-height,initial-scale=1, maximum-scale=3, minimum-scale=1, user-scalable=no"
/>
```
...
export class WKTUtil {
  public static wktToJson(wkt: string): number[][][] {
    ...
    wkt = wkt.substring(index); // 去除前边的 POLYGON
    wkt = wkt.replace(/\(\s*/g, "[");
    wkt = wkt.replace(/\s*\)/g, "]");
    wkt = wkt.replace(/,\s*/g, "],[");
    wkt = wkt.replace(/\(/g, "[");
...
如果不是特别需要, 我记得有个`WorldJson`项目能直接用, 网络上也有开源地图.
